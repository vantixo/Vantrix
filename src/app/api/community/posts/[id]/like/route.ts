 ...
 *
 * Atomically toggles the authenticated user's like on a community post.
 *
 ...
 * the toggle_community_post_like() Postgres function (migration 20241200),
...
export async function POST(_req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { data, error } = await supabaseAdmin.rpc('toggle_community_post_like', {
      p_post_id: params.id,
      p_user_id: user.id,
    });
    ...
    if (error) {
      ...
      logger.error("community:post-like-rpc-error", { error: error.message, postId: params.id });
      return NextResponse.json({ error: "Internal server error" }, { status: 500 });
    }

    if (!isLikeResult(data)) {
      logger.error("community:post-like-shape-error", { postId: params.id, data: JSON.stringify(data) });
    ...
    logger.error("community:post-like-error", { error: String(err), postId: params.id });
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
